...
__kernel void feature_detection2 (
    ...
    for (int y = -half_smoothing; y <= half_smoothing; ++y) {
        ...
        float harris_k = 0.05;
        float4 r = (float4)(0);
        r.x = ((s.x * s.y - s.z * s.z) - harris_k * (s.x + s.y) * (s.x + s.y))/ 700000000000;
        if (r.x < 209) {
            r.x=0;
        }
        r.y = r.x;
...
__kernel void feature_detection3 (
    ...
    const uint4 org = read_imageui(src_org, clamp_sampler, coord);
    ...
    for (int y = -half_supr; y <= half_supr; y++) {
        for (int x = -half_supr; x <= half_supr; ++x) {
            ...
            if (r.x > max.x) {
                write_imagef(dest, pos, convert_float4(or));
                return;
            }
        }
    }
